...
export function CategoryPills() {
  return (
    <div className="w-full bg-white border-b border-gray-100 py-2 md:hidden sticky top-0 z-30 overflow-x-auto no-scrollbar shadow-sm">
      <div className="container-custom flex gap-2 px-4 min-w-max">
        {categories.map((cat) => (
          <Link key={cat.name} href={cat.href}>
            <div 
              className={`px-3 py-1.5 rounded-full border text-[11px] font-black whitespace-nowrap transition-all cursor-pointer flex items-center gap-1
